...
static volatile int nowServing = 0;
static volatile int nextTicket = 0;


void ticketLock(volatile int* nextTicket, volatile int* nowServing) {
    int m_ticket = __sync_fetch_and_add(nextTicket, 1);
    while(m_ticket != *nowServing){
    }
}

void ticketUnlock(volatile int *nowServing) {
    ++*nowServing;
}
...
inline static void push(Object arg, int pid) {
    ...
    /* Critical section                                                       */
    /**************************************************************************/
    ticketLock(&nextTicket, &nowServing);

    n->next = Head;
    Head = n;

    ticketUnlock(&nowServing);
    /**************************************************************************/
}
