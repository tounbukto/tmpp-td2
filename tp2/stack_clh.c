...
typedef lockNode* lock_t;

volatile lock_t m_lock;

void lock(lock_t* s_lock, lockNode* m_node) {
    m_node->locked = 1;
    m_node->next = __sync_lock_test_and_set(s_lock, m_node);
    while(m_node->next->locked);
}

void unlock(lock_t* s_lock){
    (*s_lock)->locked = 0;
    *s_lock = (*s_lock)->next;
}
...
static __thread lockNode m_lock = lockNode{NULL, 0};
